...
**Date:** 2026-06-09
**Status:** Brainstorm complete (Sections 1–5, gated sign-off). Awaiting owner review of this spec before writing-plans. Compound V pre-flights to fold in.
...
| i18n | `settings:` namespace, flat dotted keys, `nsSeparator:':'`, `keySeparator:false`. Per-namespace coverage tests assert all 5 locales (`en,de,fr,es-419,pt-BR`). **Allowlist = 0** (`TRANSLATION_DEFERRED = []`). | `apps/mobile/src/i18n/`, `__tests__/coverage.test.ts:96` |
| Dev-pill pattern | `DevLocaleBar` + `StatePicker`, `__DEV__` only — template to mirror. | `apps/mobile/App.js` |
...
- **Cooldowns (frustration-14d, attempt-90d) = elapsed duration** between instants (`now` vs stored ISO), **NOT** calendar-day diffs — avoids a midnight off-by-one. Use duration-based date-fns helpers (`isAfter(addDays(...))` / hours÷24), **not** `differenceInCalendarDays`.
...
**Recorders** (in `rating-store.ts`, called from existing paths):

| Recorder | Wired into | Writes |
|---|---|---|
| `recordActiveDay()` | App boot, **after `hydrateStorage()`** | `distinctDayCount`++, `lastActiveDay` |
| `recordSuccessfulSearch()` | search success path | `successfulSearches`++ |
| `recordFrustration(source)` | 429 / upstream-quota / no_viable / error / **feedback open** | `lastFrustrationAt` |
| `recordFirstSaveDone()` | first `handleSave` success | `firstSaveDone` flag |
...
- **Fallback (required):** guard with `Linking.canOpenURL`; if no mail client → **copy the support address to clipboard + confirmation toast** (or render as selectable text). The valve must remain usable without a configured mail app.
...
- **Layered open order:** `StoreReview.storeUrl()` → hardcoded **native** store URL → hardcoded **web** store URL (browser, final fallback).
...
## 8. Section 5 — Dev/testability & Edge cases (D10, EC1–EC9)
...
1. **Add `expo-store-review`** (SDK 55-compatible). **Native module → requires a dev-client rebuild**; won't hot-reload (LG4). Verify the exact API surface (`requestReview` / `isAvailableAsync` / `hasAction` / `storeUrl`) under SDK 55 during library pre-flight (Compound V doc-validator).
2. **Reuse `storage.ts`** (AsyncStorage wrapper) for all `rating.*` keys. Confirm the synchronous-cache-on-`set()` property holds (EC3 / D7 stacking depends on it).
3. **Owner inputs** (may stub now, required at launch): support email; App Store ID + Play package name; `emailSubject` strategy (a)/(b).
...
| `apps/mobile/src/screens/MomentDetailScreen.js` | wire `maybePromptAfterSave` on save success; `recordFirstSaveDone` |
| result-view screen(s) | wire `maybePromptAfterView` on the natural break |
| search success path / 429 + error paths | `recordSuccessfulSearch` / `recordFrustration(source)` |
| `apps/mobile/App.js` | `recordActiveDay()` after `hydrateStorage()` |
...
- **Unit (Node, zero mocks):** `eligibility.test.ts` golden table — one row per `EligibilityReason`, null-history first-attempt rows, exact-threshold boundary rows (D6).
...
- **LG1** — Under forced-eligible (Debug "Force `requestReview()`"), confirm the **system card appears**. *Android leg:* the in-app review card renders **only for an app installed via Google Play** (internal test track or higher); a sideloaded dev-client shows **nothing** — **expected, not a broken wrapper**. iOS shows it in dev/TestFlight. So LG1-Android is gated on Play Console setup + an internal-track upload.
...
- **LG11 — Pre-submission compliance self-check:** re-verify the §2 guardrails — native API only; **no sentiment pre-question anywhere in any flow**; no incentive tied to rating; the Settings feedback channel is **independent and not a gate**; "Rate" never pre-selects stars.
